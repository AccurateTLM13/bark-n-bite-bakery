...
interface PupBoxSelectionModalProps {
  isOpen: boolean;
  onClose: () => void;
  products: Product[];
...
const PupBoxSelectionModal: React.FC<PupBoxSelectionModalProps> = ({
  isOpen,
  onClose,
  products,
  onConfirm,
      ...
      <div className="bg-white rounded-3xl max-w-4xl w-full max-h-[90vh] overflow-hidden shadow-floating">
        <div className="p-6 border-b border-brown-200">
          <div className="flex justify-between items-center">
            ...
            <button onClick={handleClose} className="text-brown-500 hover:text-brown-700 text-3xl">×</button>
          </div>
            ...
            {products.map((product) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="p-4">
                    <img src={product.image} alt={product.name} className="w-24 h-24 object-cover rounded-full mx-auto mb-4" />
                    <h4 className="text-lg font-bold text-brown-900 mb-2 font-serif text-center">{product.name}</h4>
                    <p className="text-brown-600 text-sm text-center leading-relaxed">{product.description}</p>
                  </div>
                </div>
              );
            })}
        ...
        <div className="p-6 border-t border-brown-200">
          <div className="flex justify-between items-center">
            <button onClick={handleClose} className="px-6 py-3 text-brown-700 hover:text-brown-900 font-semibold transition-colors">
              Cancel
            </button>
            <button
